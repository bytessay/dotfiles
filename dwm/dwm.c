...
struct Client {
	...
	int bw, oldbw;
	unsigned int tags;
	int isfixed,isglobal, isfloating, isurgent, neverfocus, oldstate, isfullscreen;
	Client *next;
	Client *snext;
	Monitor *mon;
	Window win;
...
cleanup(void)
{
    ...
    system("pkill -f /home/uavony/.dwm/autostart.sh");
}
...
drawbar(Monitor *m)
{
	...
	if ((w = m->ww - tw - stw - x) > bh) {
		if (m->sel) {
			...
			drw_text(drw, x, 0, 100, bh, lrpad / 2, m->sel->name, 0);
			if (m->sel->isfloating)
...
tile(Monitor *m)
{
	...
	for (i = my = ty = 0, c = nexttiled(m->clients); c; c = nexttiled(c->next), i++)
		if (i < m->nmaster) {
			...
			resize(c, m->wx + (n == 1 ? gappx : 0), m->wy + my + (n == 1 ? gappx : 0), mw - (2*c->bw) + (n > 1 ? gappx : 0) + (n == 1 ? -2 * gappx : 0) , h - (2*c->bw) + (n == 1 ? -2 * gappx : 0), 0);
			if (my + HEIGHT(c) < m->wh)
				my += HEIGHT(c);
		} else {
			h = (m->wh - ty) / (n - i);
